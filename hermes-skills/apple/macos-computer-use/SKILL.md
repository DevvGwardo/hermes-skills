...
Control a macOS desktop via the `computer` tool — screenshots, mouse, keyboard, scrolling, drag-and-drop. Provider-agnostic: works with any multimodal LLM API that supports standard function-calling (MiniMax, OpenAI, Anthropic via `computer_20251124`).

## Requirements

- **macOS only** — uses Quartz framework and `screencapture` command (Linux/Windows: tool is not loaded)
- **Standard function-calling** — `computer` tool registered as `computer` in the tool schema; Anthropic's `computer_20251124` type is used only when running with an Anthropic API key
- **pyautogui + pyobjc** — install with `~/venv/bin/python -m pip install -e '.[computer-use]'` (venv at `~/venv`)
